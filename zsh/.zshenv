#
# ZDOTDIR, so the rest of the zsh config lives under ~/.config/zsh. This file
# cannot move there, because zsh reads ~/.zshenv before it knows ZDOTDIR
# exists, so $ZDOTDIR/.zshenv is never sourced. .zprofile, .zshrc and
# .zsh_plugins.txt are read from ZDOTDIR once this is set.
#
...
# including multiplexer panes and anything they launch, such as nvim's language
# servers.
...
# .zshrc.local. Anything a language server or an agent has to inherit gets set
# before any interactive shell exists.
#
# Sourced before PATH is assembled on purpose, so appending to user_path_dirs
# here gets an entry the same existence check as the rest, and the same re-assert
